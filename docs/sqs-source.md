...
- Download the NAR package from [here](https://github.com/streamnative/pulsar-io-sqs/releases/download/v2.7.3.2/pulsar-io-sqs-2.7.3.2.nar).
   ...
   ```json
    {
        ...
        "archive": "connectors/pulsar-io-sqs-2.7.3.2.nar",
        "parallelism": 1,
        "configs":
        {
...
--archive pulsar-io-sqs-2.7.3.2.nar \
...
2. Copy the NAR package to the Pulsar connectors directory.
    ...
    ```
    cp pulsar-io-sqs-2.7.3.2.nar PULSAR_HOME/connectors/pulsar-io-sqs-2.7.3.2.nar
    ```
...
1. Build a new image based on the Pulsar image with the SQS source connector and push the new image to your image registry. This example tags the new image as `streamnative/pulsar-sqs:2.7.3.2`.

    ```Dockerfile
    FROM apachepulsar/pulsar-all:2.7.3.2
    RUN curl https://github.com/streamnative/pulsar-io-sqs/releases/download/v2.7.3.2/pulsar-io-sqs-2.7.3.2.nar -o /pulsar/connectors/pulsar-io-sqs-2.7.3.2.nar
    ```
...
3. Replace the `images` section in the `pulsar.yaml` file with the `images` section of `streamnative/pulsar-sqs:2.7.3.2`.
